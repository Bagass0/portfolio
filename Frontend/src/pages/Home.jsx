...
const Home = () => {
  ...
  const experiences = textes.experiences.map(exp => ({
    ...
    companyImg: exp.companyImg?.startsWith("/")
      ? exp.companyImg
      : exp.companyImg ? `/images/entreprises/${exp.companyImg.replace(/^(\.\.\/)?images\/entreprises\//, "")}` : null
  }));

  const educations = textes.educations.map(edu => ({
    ...edu,
    logo: typeof edu.logo === "string" ? iconMap[edu.logo] : edu.logo,
    schoolImg: edu.schoolImg?.startsWith("/")
      ? edu.schoolImg
      : edu.schoolImg ? `/images/ecoles/${edu.schoolImg.replace(/^(\.\.\/)?images\/ecoles\//, "")}` : null
  }));
              ...
              <motion.div className={`timeline-item ${exp.type}`} key={idx}
                ...
                <div className="timeline-content">
                  <div className="timeline-header">
                    {exp.companyImg && <img src={exp.companyImg} alt={exp.company} className="timeline-company-img" />}
                    <div>
                      <h3>{exp.title}</h3>
                  ...
                  <ul>{exp.tasks.map((task, i) => <li key={i}>{task}</li>)}</ul>
                </div>
              </motion.div>
            ))}
          </div>
          ...
          <div className="timeline">
            ...
            {educations.map((edu, idx) => (
              <motion.div className={`timeline-item ${edu.type}`} key={idx}
                ...
                <div className="timeline-content">
                  <div className="timeline-header">
                    {edu.schoolImg && <img src={edu.schoolImg} alt={edu.school} className="timeline-company-img" />}
                    <div>
                      <h3>{edu.title}</h3>
                  ...
                  <ul>{edu.tasks.map((task, i) => <li key={i}>{task}</li>)}</ul>
                </div>
              </motion.div>
            ))}
          </div>
